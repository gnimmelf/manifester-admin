var MyAccounting = (function (exports) {
'use strict';
...
var currentLocale = 'us';

var accounting = {
  currency: function currency(number) {
    return formatMoney(number, locales[currentLocale]);
  },
  number: function number(_number) {
    return formatNumber(_number, locales[currentLocale]);
  },
  unformat: function (_unformat) {
    function unformat(_x) {
      return _unformat.apply(this, arguments);
    }

    unformat.toString = function () {
      return _unformat.toString();
    };

    return unformat;
  }(function (numberString) {
    return unformat(numberString, locales[currentLocale], locales[currentLocale].decimal);
  }),
  locale: function locale(_locale) {
    if (_locale) {
      if (locales[_locale]) {
        ...
        throw new Error('Unknown locale: ' + _locale);
      }
    } else {
...
function loadLocale(name) {
    ...
    if (!locales$1[name] && (typeof module !== 'undefined') &&
...
// Because an asshole is developing `moment`...
...
hooks.locale('nb');
console.log('moment locale', hooks.locale(), hooks.weekdays());
...
//# sourceMappingURL=my-accounting.js.map
